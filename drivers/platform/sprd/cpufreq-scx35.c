...
#define BINNING_CAL_BIT                 (5)
int sci_efuse_binning_result_get_sharklc(u32 *p_binning_data)
{
	u32 data = __ddie_efuse_read(BLK_BINNING_DETA);
	u32 binning = (data >> BINNING_CAL_BIT) & 0x7;
	if (binning == 0) {
		*p_binning_data = 0;
		return -1;
	}
...
#elif defined (CONFIG_MACH_SP9830I_J3LTE) || defined (CONFIG_MACH_SP9830I_J2LTE) || defined (CONFIG_MACH_J1MINILTE_AFR)
static struct cpufreq_table_data sc9832_cpufreq_table_data = {
	.freq_tbl = {
		{0, 1500000},
		{1, 1350000},
...
#endif
static struct cpufreq_table_data sc9832_cpufreq_table_data_sharklc_ff = {
	.freq_tbl = {
		{0, 1300000},
		{1, 900000},
		{2, 768000},
...
static struct cpufreq_table_data sc9832_cpufreq_table_data_sharklc_tt = {
	.freq_tbl = {
		{0, 1300000},
		{1, 900000},
		{2, 768000},
...
static struct cpufreq_table_data sc9832_cpufreq_table_data_sharklc_ss = {
	.freq_tbl = {
		{0, 1300000},
		{1, 900000},
		{2, 768000},
...
static struct cpufreq_table_data sc9832_cpufreq_table_data_sharklc_od = {
	.freq_tbl = {
		{0, 1300000},
		{1, 900000},
		{2, 768000},
...
#if defined CONFIG_THREE_LEVEL_OF_DVFS_TABLE
...
static int sprd_freq_table_init(void)
{
	...
	if (soc_is_scx35_v0()) {
		...
		sprd_cpufreq_conf->max_axi_freq = 675000;
		if (!(sci_efuse_binning_result_get_sharklc(&p_binning_data))) {
			pr_info("Select freq table according to p_binning_data=%x\n", p_binning_data);
			if (1 == p_binning_data) {
				sprd_cpufreq_conf->freq_tbl =
					sc9832_cpufreq_table_data_sharklc_ff.freq_tbl;
				sprd_cpufreq_conf->vddarm_mv =
					sc9832_cpufreq_table_data_sharklc_ff.vddarm_mv;
			} else if (2 == p_binning_data) {
				sprd_cpufreq_conf->freq_tbl =
					sc9832_cpufreq_table_data_sharklc_tt.freq_tbl;
				sprd_cpufreq_conf->vddarm_mv =
					sc9832_cpufreq_table_data_sharklc_tt.vddarm_mv;

			} else if (3 == p_binning_data) {
				sprd_cpufreq_conf->freq_tbl =
					sc9832_cpufreq_table_data_sharklc_ss.freq_tbl;
				sprd_cpufreq_conf->vddarm_mv =
					sc9832_cpufreq_table_data_sharklc_ss.vddarm_mv;

			} else if (4 == p_binning_data) {
				sprd_cpufreq_conf->freq_tbl =
					sc9832_cpufreq_table_data_sharklc_od.freq_tbl;
				sprd_cpufreq_conf->vddarm_mv =
					sc9832_cpufreq_table_data_sharklc_od.vddarm_mv;
		...
		if (!(sci_efuse_binning_result_get(&p_binning_data))) {
			...
			#ifndef CONFIG_THREE_LEVEL_OF_DVFS_TABLE
			} else if (4 == p_binning_data) {
				sprd_cpufreq_conf->freq_tbl =
...
#if defined(CONFIG_SW_SELF_DISCHARGING)
static ssize_t cpufreq_self_discharging_show(struct kobject *kobj,
			     struct attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", self_discharging);
}

static ssize_t cpufreq_self_discharging_store(struct kobject *kobj, struct attribute *attr,
...
#endif
#if defined(CONFIG_SW_SELF_DISCHARGING)
static DEVICE_ATTR(cpufreq_self_discharging, 0660, cpufreq_self_discharging_show, cpufreq_self_discharging_store);
#endif
...
#endif
#if defined(CONFIG_SW_SELF_DISCHARGING)
	&dev_attr_cpufreq_self_discharging.attr,
#endif
	NULL,
};
