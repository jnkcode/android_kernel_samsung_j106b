...
static int gps_regulator_on(void)
{
	...
	if(of_property_read_string(root_node, "tcxo-regulator", &reg)) {
		pr_err("%s: main(3.3v) regulator field is not exsit\n", __func__);
		ret = -ENODEV;
		goto err_find_node;
	} else {
		gps_regulator = regulator_get(NULL, reg);
			if (IS_ERR(gps_regulator)){
				...
				regulator_set_voltage(gps_regulator, 3300000, 3300000);
				if(!regulator_enable(gps_regulator))
					printk("%s: (%s) main(3.3v) regulator turned ON!!\n", __func__, reg);
			}
	}
