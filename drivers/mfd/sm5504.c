...
#if defined(CONFIG_MACH_GTEXSWIFI) || defined(CONFIG_MACH_GTEXSLTE)
	if (chip->curr_status.cable_type == MUIC_SM5504_CABLE_TYPE_NONE) {
		atomic_notifier_call_chain(&muic_notifier_list,
					MUIC_DETACH_NOTI, &param);
	}
	else {
		atomic_notifier_call_chain(&muic_notifier_list,
					MUIC_ATTACH_NOTI, &param);
	}
#endif	
				       ...
				       unsigned int new_status)
{
	...
	{
		sm5504_reg_write(chip, SM5504_REG_MANUAL_SW1, 0x00);
		sm5504_reg_write(chip, SM5504_REG_MANUAL_SW2, 0x05);
		sm5504_clr_bits(chip, SM5504_REG_CONTROL, 1 << 2);
...
{
	RTINFO("SM5504 uart_en_store\n");


	if (!strncmp (buf, "1", 1)) {
		/*  UART enabled */
		enable_muic_uart = true;
	} else {
		/* UART disabled */
		enable_muic_uart = false;
	}
	return size;
}
				  ...
				  const struct i2c_device_id *id)
{
	...
	switch_dev = device_create(sec_class, NULL, (dev_t)&switch_dev, NULL, "switch");
