...
static void usb_detect_works(struct work_struct *work)
{
	...
	if (plug_in) {
		pr_info("smuic: usb detect plug in,vbus pull up\n");
		simp_charger_plugin(info);
	} else {
		pr_info("smuic: usb detect plug out,vbus pull down\n");
...
#ifdef CONFIG_MUIC_SUPPORT_RUSTPROOF
	pr_info("smuic TEST : %s, uart(%d), %d\n", __func__, is_close_uart_path, simp_charger_is_adapter());
#endif
