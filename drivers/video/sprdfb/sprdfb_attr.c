...
#ifdef CONFIG_DYNAMIC_MIPI_SUPPORT
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (panel->type != LCD_MODE_DSI) {
		...
		origin_clk = attr_info->origin_mipi_clk;
#ifdef CONFIG_HOP_FREQ_SCALING
		if (panel->info.mipi->first_hop) {
			panel->info.mipi->first_hop = false;
			attr_info->curr_mipi_clk = origin_clk;
			panel->info.mipi->phy_feq_current = origin_clk;
		}
#endif
			...
			const char *buf, size_t count)
{
	...
	pr_info("[LCD] input dphy_freq is %d\n", dphy_freq);
...
#ifdef CONFIG_HOP_FREQ_SCALING
	panel->info.mipi->dlter_freq = attr_info->origin_mipi_clk - dphy_freq;
#endif
	if (dphy_freq <= 1500000 && dphy_freq >= 90000) {
		...
		panel->info.mipi->phy_feq_current = dphy_freq;
	} else {
...
int sprdfb_create_sysfs(struct sprdfb_device *fb_dev)
{
	...
	if (panel) {
		...
		if (panel->type == LCD_MODE_DSI)
			...
			panel->info.mipi->phy_feq_current =
