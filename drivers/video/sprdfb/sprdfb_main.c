...
unsigned long g_dispc0_base_addr = 0;
#endif
#endif
...
#endif
#ifdef CONFIG_FB_SC9001
unsigned long kernel_logo_base;
uint32_t kernel_logo_size;
static void setup_logo_mem(struct sprdfb_device *dev,struct platform_device *pdev)
{
	uint32_t reserve_mem_logo[4];
	int ret;

	ret = of_property_read_u32_array(pdev->dev.of_node, "sprd,fb_mem_logo",
		reserve_mem_logo, 4);
	if(0 != ret)
		printk(KERN_ERR "sprdfb: Failed to got logo memory from dt file\n");

	kernel_logo_base = reserve_mem_logo[0];
	kernel_logo_base = kernel_logo_base << 32;
	kernel_logo_base |= reserve_mem_logo[1];
	kernel_logo_size = reserve_mem_logo[3];

        printk("sprdfb: setup_logo_mem--smem_start:%lx,reserved len:%d\n",
		kernel_logo_base,kernel_logo_size);
}
#endif
...
#ifdef CONFIG_FB_SC9001
        dev->fb->fix.smem_start = dev->fb->fix.smem_start << 32;
        dev->fb->fix.smem_start |= reserve_mem[1];
        printk("sprdfb: setup_fb_mem--smem_start:%lx,len:%d,reserved len:%d\n",
		dev->fb->fix.smem_start,len,reserve_mem[3]);
#else
		printk("sprdfb: setup_fb_mem--smem_start:%lx,len:%d,reserved len:%d\n",
...
#ifdef CONFIG_FB_SC9001
        g_dispc0_base_addr = (unsigned long)ioremap_nocache(0x63200000, 0x1000);
        if(!g_dispc0_base_addr)
            BUG();
        printk("sprdfb: set g_dispc0_base_addr = %ld\n", g_dispc0_base_addr);
#endif
#endif
	}
