...
static bool init_touch(struct bt432_ts_info *info, bool forced);
...
void tsp_charger_enable(int status)
{
	...
	dev_info(&misc_info->client->dev, "TA %s\n",
		status == 1 ? "disconnected" : "connected");
}
...
static int get_fw_info(struct bt432_ts_info *info)
{
	...
	ret = request_firmware(&fw, info->pdata->fw_name, &info->client->dev);
	if (ret) {
		dev_err(&info->client->dev, "failed to request firmware!\n");
		ret = -EINVAL;
		goto err1;
	}
...
static void fw_update(void *device_data)
{
	...
	case BUILT_IN:
		ret = get_fw_info(info);
		if (ret) {
			dev_err(&client->dev, "%s: failed to get build-in fw\n", __func__);
			return;
		}
	...
	case UMS:
		...
		fp = filp_open(info->pdata->ext_fw_name, O_RDONLY, 0);
		if (IS_ERR(fp)) {
			dev_err(&client->dev,
				"file %s open error:%d\n", info->pdata->ext_fw_name, (s32)fp);
			info->factory_info->cmd_state = 3;
			goto err_open;
		}
...
factory_ts_dev = device_create(sec_class, NULL, 0, info, "tsp");
	...
	if (info->pdata->support_tkey) {
		factory_tk_dev = device_create(sec_class, NULL, 1, info, "sec_touchkey");
...
static int bt432_ts_probe_dt(struct device_node *np,
			 struct device *dev, struct bt432_ts_platform_data *pdata)
{
	int ret;
	...
	if (ret < 0) {
		pr_err("%s: failed to request gpio_irq\n", __func__);
		return ret;
	}
	...
	ret = of_property_read_string(np, "bt432,fw_name", &pdata->fw_name);
	if (ret < 0) {
		pr_err("%s: failed to get built-in firmware path\n", __func__);
		return -EINVAL;
	}

	ret = of_property_read_string(np, "bt432,ext_fw_name", &pdata->ext_fw_name);
	if (ret < 0) {
		pr_err("%s: failed to get external firmware path\n", __func__);
		return -EINVAL;
	}
...
/* FW version read from tsp */
	...
	ret = get_fw_info(info);
	if (ret) {
		dev_err(&client->dev, "failed to firmware from binary\n");
