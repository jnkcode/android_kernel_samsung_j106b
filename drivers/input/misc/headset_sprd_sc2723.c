...
#if defined (USE_HEADSET_WATERPROOF)
...
#if defined (USE_HEADSET_WATERPROOF)
#define AUDIO_HEAD2ADC_SEL_GND_DET    (2)
...
#if defined (USE_HEADSET_WATERPROOF)
/*ANA_HDT1*/
#define AUDIO_GND_DET_PU_PD (BIT(15))
...
/*ANA_PMU1*/
#define AUDIO_BG_EN (BIT(15))
#define AUDIO_BG_IBIAS_EN (BIT(14))
#define AUDIO_VCM_EN (BIT(13))
#endif
...
static int sprd_headset_power_get(struct device *dev, struct regulator **regu, const char *id)
{
    if (!*regu) {
...
static int sprd_headset_headmic_bias_control(struct device *dev, int on)
{
    ...
    if (on) {
        PRINT_INFO("sprd_headset_headmic_bias_control : ON\n");
        ret = regulator_enable(sprd_hts_power.head_mic);
    } else {
        PRINT_INFO("sprd_headset_headmic_bias_control : OFF\n");
...
void headset_irq_detect_enable_ext(int enable)
{
	...
	if (!ht) {
		PRINT_ERR("ht is NULL!\n");
		return;
	}

	headset_irq_detect_enable(enable, ht->irq_detect);
}
...
static int headset_plug_confirm_by_adc(int last_gpio_detect_value)
{
    ...
    PRINT_INFO("headset_plug_confirm_by_adc success!! adc_last=%d\n", adc_last);
    return adc_current;
}
...
no_mic_retry:
...
#if defined (USE_HEADSET_WATERPROOF)
        headset_reg_set_bit(HEADMIC_DETECT_REG(ANA_PMU1), AUDIO_VCM_EN | AUDIO_BG_IBIAS_EN | AUDIO_BG_EN);
...
#if defined (USE_HEADSET_WATERPROOF)
        if(adc_left_average > pdata->adc_threshold_left_average) {
            PRINT_INFO("adc_left_average for water = %d\n", adc_left_average);
            headset_reg_clr_bit(HEADMIC_DETECT_REG(ANA_HDT1), AUDIO_HEAD_L_INT_PU_PD);
            PRINT_INFO("water injection  bit14 clear \n");
            return HEADSET_WATER;
        }
#endif
    } 
    else 
    ...
    PRINT_INFO("adc_left_average is %d\n", adc_left_average);

    //get adc value of mic
    set_adc_to_headmic(1);

#if defined (USE_HEADSET_WATERPROOF)
    /* Enable the headset bias */
    msleep(50); //100
    ...
    PRINT_INFO("adc_mic_average = %d  adc_left_average = %d \n", adc_mic_average, adc_left_average);
    ...
    else if (adc_mic_average > pdata->adc_threshold_4pole_detect)
...
static void headset_detect_work_func(struct work_struct *work)
{
    ...
    PRINT_INFO("headset_detect_work_func : Headset Detected !! \n");
    ...
    if(1 == plug_state_current && 0 == plug_state_last) {
...
#if defined (USE_HEADSET_WATERPROOF)
            case HEADSET_WATER:
                    PRINT_INFO("headset_type = %d (HEADSET_WATER)\n", headset_type);
                    goto out1;
#endif
            ...
            if (ht->headphone) {
                    ...
                    PRINT_INFO(" headmicbias_power off  01  3 pole\n");
#endif
                    ...
                    PRINT_INFO("headphone(3P) plug in (headset_detect_work_func)\n");
            } else {
                    headset_button_irq_threshold(1);
                    ...
                    switch_set_state(&ht->sdev, ht->type);
                    PRINT_INFO("headset(4P) plug in (headset_detect_work_func)\n");
#ifdef ADPGAR_BYP_SELECT
                    queue_delayed_work(adpgar_byp_select_work_queue, &adpgar_byp_select_work, msecs_to_jiffies(500));
#endif
            }

            /* KSND Estimate Headset detection time */
            ht->ts_after = current_kernel_time();
            ht->ts.tv_nsec = ht->ts_after.tv_nsec - ht->ts.tv_nsec;
            if( ht->ts.tv_nsec < 0) {
                --ht->ts.tv_sec;
                ht->ts.tv_nsec += 1000000000L;
            }
            pr_info("%s: detect time : %d ms\n", __func__, (int)ht->ts.tv_nsec/1000000 );
            ...
            headmicbias_power_on(&this_pdev->dev, 0);
            PRINT_INFO("headphone plug out (headset_detect_work_func) headmicbias off\n");

            if (ht->headphone) {
                PRINT_INFO("headphone(3P) plug out (headset_detect_work_func)\n");
            } else {
                PRINT_INFO("headset(4P) plug out (headset_detect_work_func)\n");
            }
...
out:
#if defined (USE_HEADSET_WATERPROOF)
    headset_reg_clr_bit(HEADMIC_DETECT_REG(ANA_HDT1), AUDIO_GND_DET_PU_PD);
    headset_reg_set_bit(HEADMIC_DETECT_REG(ANA_HDT1), AUDIO_HEAD_L_INT_PU_PD);
    headset_reg_set_bit(HEADMIC_DETECT_REG(ANA_PMU0), AUDIO_PLGPD_EN);
    PRINT_INFO("ANA_HDT1 0x%04x\n", sci_adi_read(HEADMIC_DETECT_REG(ANA_HDT1)));
out1:
#endif
    if(0 == plug_state_last) {
        headmicbias_power_on(&this_pdev->dev, 0);
        //PRINT_INFO("headmicbias off at the end of headset-detect-work-func\n");
    }

    headset_irq_detect_enable(1, ht->irq_detect);
    PRINT_INFO("headset_detect_work_func end \n");
...
static irqreturn_t headset_detect_irq_handler(int irq, void *dev)
{
    ...
    /* KSND Get Current time of detecting headset */
    ht->ts = current_kernel_time();
    ...
    printk("headset_detect_irq_handler: IRQ_%d(GPIO_%d) = %d, ANA_STS0 = 0x%08X\n",
    ...
    queue_work(ht->detect_work_queue, &ht->work_detect);
    return IRQ_HANDLED;
}
...
#if defined (SEC_USE_HS_SYSFS)
static ssize_t state_switch_show(struct device *dev, 
...
static DEVICE_ATTR(mic_adc, S_IRUGO , mic_adc_show, NULL);


#if defined (SEC_SYSFS_ADC_EARJACK)
/*************************************
 * add sysfs for factory test. KSND
 *************************************/
static ssize_t jacks_adc_show(struct device *dev, 
                                      struct device_attribute *attr, char *buf)
{
    struct sprd_headset *ht = &headset;
    struct sprd_headset_platform_data *pdata = ht->platform_data;
    ...
    val[1] = pdata->adc_threshold_3pole_detect;
    val[2] = pdata->adc_threshold_4pole_detect;
                                       ...
                                       struct device_attribute *attr, char *buf)
{
    ...
    val[0] =  pdata->headset_buttons[0].adc_min;
    val[1] =  pdata->headset_buttons[0].adc_max;
                                       ...
                                       struct device_attribute *attr, char *buf)
{
    ...
    val[0] =  pdata->headset_buttons[1].adc_min;
    val[1] =  pdata->headset_buttons[1].adc_max;
                                       ...
                                       struct device_attribute *attr, char *buf)
{
    ...
    val[0] =  pdata->headset_buttons[2].adc_min;
    val[1] =  pdata->headset_buttons[2].adc_max;
                                       ...
                                       struct device_attribute *attr, char *buf)
{
    ...
    val[0] =  pdata->headset_buttons[3].adc_min;
    val[1] =  pdata->headset_buttons[3].adc_max;

    return sprintf(buf, "%d %d\n",val[0],val[1]);
}

//Read Only
static DEVICE_ATTR(jacks_adc, S_IRUGO, jacks_adc_show, NULL);
static DEVICE_ATTR(send_end_btn_adc, S_IRUGO, hook_adc_show, NULL);
static DEVICE_ATTR(voc_assist_btn_adc, S_IRUGO, voc_assist_adc_show, NULL);
...
static struct sprd_headset_platform_data *headset_detect_parse_dt(struct device *dev)
{
    ...
    ret = of_property_read_u32(np, "adc_threshold_gnd_average", &pdata->adc_threshold_gnd_average);
    if(ret) {
        dev_err(dev, "fail to get irq_trigger_level_button\n");
        goto fail;
    }

    ret = of_property_read_u32(np, "adc_threshold_left_average", &pdata->adc_threshold_left_average);
    if (ret) {
        dev_err(dev, "fail to get irq_trigger_level_button\n");
        goto fail;
    }
...
static int headset_detect_probe(struct platform_device *pdev)
{
    ...
    INIT_WORK(&ht->work_detect, headset_detect_work_func);
    //ht->detect_work_queue = create_singlethread_workqueue("headset_detect");
    ht->detect_work_queue = alloc_workqueue(("headset_detect"), WQ_MEM_RECLAIM|WQ_HIGHPRI, 1);
    ...
    if (ret < 0) {
        ...
        /* Disable the detecting irq here. And enable it
         * in sprd-codec.c(after audio pa short check).
         */
        headset_irq_detect_enable(0, ht->irq_detect);
    ...
    if (ret) {
        pr_err("mic_adc dev_register failed!\n");
        return ret;
    }

#if defined (SEC_SYSFS_ADC_EARJACK)
    ret = device_create_file(earjack, &dev_attr_jacks_adc);
    if (ret) {
        pr_err("key_state dev_register failed!\n");
        return ret;
    }
    ...
    ret = device_create_file(earjack, &dev_attr_voc_assist_btn_adc);
    if (ret) {
        pr_err("key_state dev_register failed!\n");
        return ret;
    }
