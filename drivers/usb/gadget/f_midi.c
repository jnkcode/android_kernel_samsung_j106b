...
static struct usb_interface_descriptor ac_interface_descriptor /* __initdata */ = {
...
static struct uac1_ac_header_descriptor_1 ac_header_descriptor /* __initdata */ = {
...
f_midi_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (status < 0)
		goto fail;
	ac_interface_descriptor.bInterfaceNumber = status;

	status = usb_interface_id(c, f);
	if (status < 0)
		goto fail;
	ms_interface_desc.bInterfaceNumber = status;
	ac_header_descriptor.baInterfaceNr[0] = status;
	...
	midi_function[i++] = (struct usb_descriptor_header *) &ac_interface_descriptor;
	midi_function[i++] = (struct usb_descriptor_header *) &ac_header_descriptor;
