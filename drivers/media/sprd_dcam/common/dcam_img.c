...
LOCAL int sprd_img_opt_flash(struct dcam_frame *frame, void* param)
{
	...
	info = &dev->dcam_cxt;
	flash_status = info->flash_status;
	if (info->flash_status < FLASH_STATUS_MAX) {
		...
		if(flash_status == info->flash_status)
...
#if defined(CONFIG_MACH_J3XLTE) ||  defined(CONFIG_MACH_J3XNLTE) || defined(CONFIG_MACH_J2XLTE) || defined(CONFIG_MACH_J3X3G) || defined(CONFIG_MACH_J1ACEVELTE) || defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_GTEXSLTE) || defined(CONFIG_MACH_J1MINIVE3G)
...
ssize_t sprd_img_read(struct file *file, char __user *u_data, size_t cnt, loff_t *cnt_ret)
{
	...
	case SPRD_IMG_GET_SCALE_CAP:
		...
		read_op.parm.reserved[3] = DCAM_SC_COEFF_UP_CAPTURE_MODE_MAX;
