...
STATIC_FUNC void _tasklet(unsigned long param)
{
	...
	if (!host->mrq) {
		spin_unlock_irqrestore(&host->lock, flags);
		printk("%s(%s) host->mrq is NULL, return\n", __func__, host->deviceName);
		return;
	}
	...
	if (onOff) {
		if (!host->_1_8V_signal_enabled) {
			if (regulator_is_enabled(host->_1_8V_signal)) {
					...
					printk("sdhost %s signal voltage has enabled!\n", host->deviceName);
			}
			else{
		...
		if (host->_1_8V_signal_enabled) {
			if (!regulator_is_enabled(host->_1_8V_signal)){
				...
				printk("sdhost %s signal voltage has disabled!\n", host->deviceName);
			}
			else{
...
STATIC_FUNC int _getBasicResource(struct platform_device *pdev, struct sdhost_host *host)
{
	...
	if (soc_is_scx9832a_v0()) {
		host->writeDelay = 0x18;
		host->readPosDelay = 0x0c;
		host->readNegDelay = 0x0c;
	}
#endif
